...
namespace AndreTurismAPP.PackageServices.Services
{
    public class TicketService
    {
        ...
        public static async Task<Ticket>? PosTicket(Ticket ticket)
        {
            try
            {
                HttpResponseMessage response = await TicketService.client.PostAsJsonAsync("https://localhost:7251/api/Tickets", ticket);
                response.EnsureSuccessStatusCode();
                string ender = await response.Content.ReadAsStringAsync();
